...
from generators import OASIS_Generator
from discriminators import OASIS_Discriminator
import os
import copy
import torch
...
def preprocess_input(opt, data):
    ...
    #     img=img.to(device)
    label_map = torch.tensor(label,dtype=torch.long).to(device)
    bs, _, h, w = label_map.size()
    nc = 30
    # if opt.gpu_ids != "-1":
    #     input_label = torch.LongTensor(bs, nc, h, w).zero_().to(device)
    # else:
    img = img.to(device)
    input_label = torch.FloatTensor(bs, nc, h, w).zero_().to(device)
    # print(input_label.shape)
    input_semantics = input_label.scatter_(1, label_map, -1)
