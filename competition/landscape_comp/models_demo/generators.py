...
class OASIS_Generator(nn.Module):
    def __init__(self, opt):
        ...
        if not self.opt.no_3dnoise:
            self.fc = nn.Conv2d(self.semantic_nc + self.opt.z_dim, 16 * ch, 3, padding=1)
        else:
            self.fc = nn.Conv2d(self.semantic_nc, 16 * ch, 3, padding=1)

    def compute_latent_vector_size(self, opt):
        w = self.crop_size // (2**(opt.num_res_blocks-1))
        h = round(w / self.aspect_ratio)
        return h, w
...
class ResnetBlock_with_SPADE(nn.Module):
    def __init__(self, fin, fout, opt):
        ...
        spade_conditional_input_dims = self.semantic_nc
